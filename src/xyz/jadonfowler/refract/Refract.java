		...
		if(c==3) {
			...
			if(c == '[') {
				...
				if(Math.floor(x) != Math.ceil(x)) {
					throw new NumberFormatException("X is not an Integer!");
				}
			...
			if(!isInteger(y) || !isInteger(x)) {
				throw new NumberFormatException("X or Y is not an Integer!");
			}
			...
			if(!isInteger(y) || !isInteger(x) || !isInteger(v)) {
				throw new NumberFormatException("X, Y or V is not an Integer!");
			}
			...
			System.out.err("Unknown command: " + c);
		}
	}
...
enum Directions {
	RIGHT, LEFT, DOWN, UP,
	UP_RIGHT, DOWN_RIGHT, UP_LEFT, UP_RIGHT;
...
class Stack {
	...
	public void action(char c) {
		...
		case '{':
			...
			break;
		case 'r':
			if(refact.debug) System.out.println("REVERSE");
...
class FileIO {
	...
	public void writeData(String[] data) throws FileNotFoundException, IOException {
		synchronized(this.file) {
			...
			for(String line : data) sb.append(line + newLine);	
